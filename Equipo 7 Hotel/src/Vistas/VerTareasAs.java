...
class VerTareasAs extends JFrame implements ActionListener, MouseListener {
    EmpleadoMotor aux;
    ...
    Color color, fuente;
    String tipoemp;
    String[] columnNames = {"Fecha", "Tarea", "Empleado",};
    int id_tipo;
    
    public VerTareasAs() throws ClassNotFoundException{
            ...
            for(int j=0; j<Empleado.size(); j++){
                if(Tarea.get(i).getId_empleado()==Empleado.get(j).getId())
                    modelo.addRow(new Object[]{Tarea.get(i).getFecha(),Tarea.get(i).getTarea(),
                        Empleado.get(j).getNombre() +" "+Empleado.get(j).getApellidos()});
            }           
        }   
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        nombre_txt.setText("Empleado: "+name);
